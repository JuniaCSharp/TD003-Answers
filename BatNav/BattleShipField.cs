...
namespace BatNav
{
	...
	public class BattleShipField  
	{
		...
		public EventArgs e;
		// création d'un délégué servant de prototype aux fonctions capable de prendre en charge l'évenement
		...
		{
			// création d'un nouvelle ID pour cette partie
			gameID = Guid.NewGuid();
        ...
        {
			List<Player> playersList;
			string pseudo = GamesManager.GetGamerPseudo(gamerID);
			playersList = (enemy) ? playersListB : playersListA;
			playersList.Add(new Player(pseudo, gamerID, NbStrikes, AppDef.PlayerStatus.NotSet));
		...
		public AppDef.State ProcessStrike(Guid playerID, SeaElement strikeElt)
		{
			...
			if ((thePlayer = playersList.Find(player => player.ID == playerID)) != null)
			{
				...
				var query = impactedSeaElements.Where(seaElt => seaElt == strikeElt);

				if (query.Count() == 0)
				{
					Boolean test = impactedSeaElements.TryAdd(strikeElt);
				}
